...
def subtreeExtraction(sentence):
    ...
    for token_pos in (range(len(doc))):
        list1 = []
        #print("Token: {}".format(sentence[token_pos]))
        list1.append("Subtree for token '{}': ".format(doc[token_pos]))
...
def spanHead(span):
    ...
    for i in span_as_doc:
        if(i.text == i.head.text):
            head = i.text
    return head
